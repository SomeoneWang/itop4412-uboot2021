...
int ext2fs_format(struct blk_desc  *dev_desc, struct disk_partition info, char set_journaling)
{
	...
	for (bgcnt = 0; bgcnt < blk_group_num; bgcnt++) {
		...
		*/
		...
		if (have_rev_gdt) {
			start_block += 8;
			//if (dev_desc->block_write(dev_desc->dev, start_block, 8, (ulong *)img2) != 8) {
			if (blk_dwrite(dev_desc, start_block, 8, (ulong *)img2) != 8) {
		...
		if (bgcnt == 0) {
			...
			for (rev_cnt = 2; rev_cnt <= reserved_gdt_blk +
				...
				//if (dev_desc->block_write(dev_desc->dev, start_block, 8, (ulong *)reserve_img) != 8) {
				if (blk_dwrite(dev_desc, start_block, 8, (ulong *)reserve_img) != 8) {
		...
		*/
		...
		//if (dev_desc->block_write(dev_desc->dev, start_block, 8, (ulong *)img3) != 8) {
		if (blk_dwrite(dev_desc, start_block, 8, (ulong *)img3) != 8) {		
			printf("Can't write Descriptor Table(%d)~~~!!!\n", bgcnt);
		}
		...
		*/
		...
		//if (dev_desc->block_write(dev_desc->dev, start_block, 8,(ulong *)img4) != 8) {
		if (blk_dwrite(dev_desc, start_block, 8,(ulong *)img4) != 8) {	
		...
		for (j = 0; j < (indt_blknum /80) + 4; j++) { // 2 means root + a 
			start_block += j * 8;
			//if (dev_desc->block_write(dev_desc->dev, start_block, 640, (ulong *)zerobuf) != 640) {
			if (blk_dwrite(dev_desc, start_block, 640, (ulong *)zerobuf) != 640) {	
...
int ext_format (int argc, char *const argv[], char filesystem_type)
{
	...
	printf("Start format %s%d partition%d ...\n", argv[1], fs_dev_desc->devnum, fs_dev_part);
	
	switch (filesystem_type) {
	case FS_TYPE_EXT3:
	case FS_TYPE_EXT2:
		if (ext2fs_format(fs_dev_desc, fs_partition, filesystem_type) != 0)
...
int do_ext2_format(struct cmd_tbl *cmdtp, int flag, int argc,
		      char *const argv[])
{
...
int do_ext3_format(struct cmd_tbl *cmdtp, int flag, int argc,
		      char *const argv[])
{
